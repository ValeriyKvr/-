...
namespace ConsoleApp4
{
    public class PolishLook
    {
        public static string ToPolishLook(string input)
        {
            ...
            for (int i = 0; i < input.Length;)
            {
                ...
                if (symbol == ")")
                {
                    ...
                    {
                        return "Вираз введений не коректно";
                    }
                    else
                    {
                        lexemas.Pop();
                ...
                if (CheckSymbol.IsOperator(symbol))
                {
                ...
                if (CheckSymbol.IsNumber(symbol))
                {
                    output += symbol;
                }
                output += ' ';
        ...
        {
            string symbol = "";
            if (CheckSymbol.IsOperator(input[i].ToString()))
            {
                symbol += input[i];
                i++;
                return symbol;
            ...
            if (CheckSymbol.IsDigit(input[i]))
            {
                while (i < input.Length && CheckSymbol.IsDigit(input[i]))
                {
                    symbol += input[i];
                    i++;
                }
            ...
            if (CheckSymbol.IsLetter(input[i]))
            {
                while (i < input.Length && CheckSymbol.IsLetter(input[i]) && !(CheckSymbol.IsOperator(symbol)))
                {
                    symbol += input[i];
                    i++;
                }
